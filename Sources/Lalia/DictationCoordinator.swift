...
@MainActor final class DictationCoordinator: ObservableObject {
    ...
    func enableDictation() async {
        ...
        do { message = "Preparing Speech…"; try await transcriber.prepare(); speechPrepared = true; phase = .idle; message = "Ready — hold ⇧⌘D to dictate." }
        catch { phase = .idle; message = error.localizedDescription }
    }
    func hotkeyPressed() {
        guard phase == .idle, speechPrepared, authorizer.currentStatus().ready else { return }
